...
public class PacketRemoveTargetingEntity
{
    private static final Logger LOGGER = LogManager.getLogger();
    private int targetEntity;

    public PacketRemoveTargetingEntity(FriendlyByteBuf buf) {
        targetEntity = buf.readVarInt();
    }

    public PacketRemoveTargetingEntity(int targetEntity) {
    ...
    public boolean handle(Supplier<NetworkEvent.Context> ctx) {
        ...
        if (sideReceived != LogicalSide.SERVER) {
            LOGGER.warn("PacketRemoveTargetingEntity received on wrong side:" + sideReceived);
            return false;
        }
        ...
        if (sendingPlayer == null) {
            LOGGER.warn("ServerPlayerEntity was null when PacketRemoveTargetingEntity was received");
            return false;
        }

        context.enqueueWork(() -> {
            sendingPlayer.getCapability(ISkyrimPlayerDataProvider.SKYRIM_PLAYER_DATA_CAPABILITY).ifPresent((cap) -> {
                cap.removeTargetingEntity(targetEntity);
